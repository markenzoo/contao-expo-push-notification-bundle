<?php

declare(strict_types=1);

/*
 * This file is part of markenzoo/contao-expo-push-notification-bundle.
 *
 * Copyright (c) 2020 markenzoo eG
 *
 * @package   markenzoo/contao-expo-push-notification-bundle
 * @author    Felix Kästner <kaestner@markenzoo.de>
 * @copyright 2020 markenzoo eG
 * @license   https://github.com/markenzoo/contao-expo-push-notification-bundle/blob/master/LICENSE MIT License
 */

$GLOBALS['TL_LANG']['tl_user_group']['expo_push_notifications_legend'] = 'Push Benachrichtigungen';

$GLOBALS['TL_LANG']['tl_user_group']['expo_push_notification'] = ['Push Benachrichtigungen Rechte', 'Hier können Sie die einzelne Rechte der Push Benachrichtigungen festlegen.'];

$GLOBALS['TL_LANG']['tl_user_group']['reference'] = [
    'delete' => 'Löschen',
    'create' => 'Anlegen',
    'edit' => 'Bearbeiten',
];
